      ...
      <Stack spacing={3} sx={{ px: 3, my: 5 }}>
        {data.map((progress) => (
          <LinearProgress
            variant="determinate"
            key={progress.status}
            value={progress.value ? progress.value : 0}
            color={
              (progress.status === 'Pending' && 'warning') ||
              (progress.status === 'Cancel' && 'error') ||
              'success'
            }
            sx={{ height: 8, backgroundColor: (theme) => alpha(theme.palette.grey[500], 0.16) }}
          />
        ))}
      </Stack>
            ...
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
              <Box
                sx={{
                  ...
                  ...(progress.status === 'Pending' && { backgroundColor: 'warning.main' }),
                  ...(progress.status === 'Cancel' && { backgroundColor: 'error.main' }),
                }}
              />
